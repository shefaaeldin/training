...
@section('content')
                        ...
                        <form method="post" class="form-horizontal" action="/city/{{$city->id}}">
                           @csrf
                           @method('PUT')
                                 <div class="form-group" style="margin-top:20px;"><label class="col-sm-2 control-label">Name</label>
                                    <div class="col-sm-10"><input type="text" class="form-control" name="name" value="{{old('name')? old('name') : $city->name}}"> 
                                         @if ($errors->has('name'))
